...
# For enforce some appliance to stay off during specific time periods of the 
# day there is the binary parameter 'o' (on_off file, if it doesn't exist then
# all all the o[t, i] will be set to 1)
param o {t in 1..T, i in 1..N} binary;
...
minimize objective_binary:
	...
	lambda1 * sum{t in 2..T, i in 1..N}(w[i] * sum{j in 1..S[i]}((x[t, i, j] - x[t-1, i, j]) * o[t, i])^2) +
...
s.t. link_transition {t in 2..T, i in 1..N, j in 1..S[i]}:
	u[t, i, j] - d[t, i, j] = (x[t, i, j] - x[t-1, i, j]) * o[t, i];
...
s.t. maximum_appliance_consumption1 {i in 1..N}:
	sum{t in (Tzone1+1)..Tzone2, j in 1..S[i]}(p[i, j] * x[t, i, j] * o[t, i]) <= m[1, i];

s.t. maximum_appliance_consumption2 {i in 1..N}:
	sum{t in 1..Tzone1, j in 1..S[i]}(p[i, j] * x[t, i, j] * o[t, i]) + 
	sum{t in (Tzone2+1)..T, j in 1..S[i]}(p[i, j] * x[t, i, j] * o[t, i]) <= m[2, i];
...
s.t. total_consumption{t in 1..T}:
	sum{i in 1..N, j in 1..S[i]}(p[i, j] * x[t, i, j] * o[t, i]) <= y[t];
